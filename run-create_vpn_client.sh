...
if [ -z $OPENVPN_CLIENT ]; then
    ...
    if [ -z $OPENVPN_PORT ]; then
        ...
        echo "..."
        ansible-playbook -i prod-inventory -vv --ask-sudo-pass --extra-vars="OPENVPN_CLIENTS=$OPENVPN_CLIENT" openvpn-client.yml
    else
        echo "Usando OPENVPN_PORT: $OPENVPN_PORT"
        echo "..."
        ansible-playbook -i prod-inventory -vv --ask-sudo-pass --extra-vars="OPENVPN_CLIENTS=$OPENVPN_CLIENT OPENVPN_PORT=$OPENVPN_PORT" openvpn-client.yml
    fi
fi